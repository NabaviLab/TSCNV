def DetectFirstCP(R, epsilon):
    ...
    for i in loop_range:
        if R[i-1] >= F_lastlow:
            ...
            while ((seglow > segnum) and (F_lastlow >= F(firstlowind[seglow-2]))):
                seglow -= 1
            ...
            if seglow == segnum:
                ...
                if indsegnum == i:
                    ...
                    F[firstlowind[seglow-1]-1] = F_lastlow
                seglow += 1
        ...
        if seglow == segnum:
            if F_up_first >= F_lastlow:
                ...
                while ((segup > segnum) and (F_lastup <= F[firstupind[segup-2]])):
                    ...
                    F_lastup = (F[firstupind[segup-1]-1] + (F_lastup - F[firstupind[segup-1]-1]) *
	        			        ((i - firstupind[segup]+1) / (i - firstupind[segup-1]+1)))
                F[firstupind[segup-1]-1:i] = F_lastup
                while segnum < segup:
                    segnum += 1
                    F[indsegnum-1:firstupind[segnum-1]-1] = F_up_first
                    indsegnum = firstupind[segnum-1]
